...
- **Application Management Hover Navigation System** (July 05, 2025): Converted Application Management from tab-based to hover link navigation system for both main navigation and Mappings sub-navigation. Main menu items (User Management, Role Management, List Values, Workflow Designer, Workflow Templates, Mappings, User Role Report) now use hover buttons with smooth transitions. Mappings features dropdown hover menu with User & Role Mapping and Role-Action Mapping options. Improved user experience with cleaner interface and better visual hierarchy.
