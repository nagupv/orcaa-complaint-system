...
- **User Management Status Indicators & Workflow Task Recovery** (July 05, 2025): Enhanced User Management interface with status column displaying "Active" vs "Pending Activation" badges for clear user account state visibility. Fixed missing workflow task issue for complaint AQ-2025-002 by implementing manual workflow task creation route and database recovery. Added comprehensive workflow task generation with corresponding inbox items for proper task assignment and visibility. Enhanced audit trail tracking for manual workflow interventions.
